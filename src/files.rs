...
pub fn create_app_layout(project_path: &Path, project_name: &str) -> Result<()> {
...
mod tests {
    ...
    fn create_test_config(name: &str, use_turbo: bool) -> ProjectConfig {
        ProjectConfig::new(name.to_string(), true, use_turbo)
    }

    #[test]
    fn test_create_package_json_without_turbo() {
        let temp_dir = setup_test_dir();
        let config = create_test_config("test-project", false);
    ...
    fn test_create_package_json_with_turbo() {
        let temp_dir = setup_test_dir();
        let config = create_test_config("turbo-project", true);
    ...
    fn test_create_app_layout() {
        ...
        let result = create_app_layout(temp_dir.path(), "my-awesome-app");
        assert!(result.is_ok());
    ...
    fn test_create_button_component() {
        ...
        let content = fs::read_to_string(temp_dir.path().join("src/components/Button.tsx")).unwrap();
